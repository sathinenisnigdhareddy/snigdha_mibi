(function ($) {
  ...
  jQuery(document).ready(function ($) {
    ...
    $('#responsive_price').val(
      '$' +
        $('#responsive_range_price').slider('values', 0) +
        ' - ' +
        ' $' +
    ...
    */
      ...
      target.find('input , textarea , select').each(function () {
        if ($(this).hasClass('require')) {
          if ($(this).val().trim() == '') {
            check = 1;
            $(this).focus();
            targetResp.html('Please enter a message');
            $(this).addClass('error');
            return false;
          } else {
    ...
    $('.submitForm').on('click', function () {
      ...
      if (check == 0) {
        ...
        request.onreadystatechange = () => {
          if (request.readyState === 4) {
            if (request.status === 200) {
              ...
              errroTarget.html(
                '<p style="color:green;">Thanks for contacting us.</p>'
              );
            }
        ...
        const data_js = {
          access_token: 'wdq9w51m6ucnhgrvgjem25mo',
          subject: 'Website Contact Form',
          text: `Name: ${$('#form-name')['0'].value}\nMobile: ${
            $('#form-mobile')['0'].value
          }\nEmail: ${$('#form-email')['0'].value}\nMessage: ${
            $('#form-message')['0'].value
          }\n`,
        };
